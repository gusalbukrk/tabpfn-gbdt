"""
==============================================================================
Script: batch_bi_metric_evaluation.py

Description:
Executes a robust Bi-Metric Geometric evaluation (Silhouette Score and
k-NN Topological Probe) natively on high-dimensional TabPFN embeddings,
paired with UMAP topological visualization.

Features Live Checkpointing: Writes/updates the CSV receipt immediately
after every dataset to prevent data loss on crash.
==============================================================================
"""
...
SUMMARY_CSV = OUTPUT_DIR / "intrinsic_evaluation_metrics.csv"
RANDOM_STATE = 42
# ==============================================================================
...
def main():
    ...
    print(
        f"Found {len(npz_files)} archives. Commencing Bi-Metric profiling with Live Checkpointing..."
    )

    for idx, filepath in enumerate(npz_files, 1):
        start_time = time.time()
        print(f"\n[{idx}/{len(npz_files)}] Evaluating: {filepath.name}")
        ...
        try:
            ...
            if "X_train_embed" not in archive or "y_train" not in archive:
                ...
                continue
            X_embed = archive["X_train_embed"].astype(np.float32)
        ...
        # Make sure target arrays match clean integer indexing for neighborhood matrices
        _, y_discrete_encoded = np.unique(y_discrete, return_inverse=True)

        # --- METRIC 1: HIGH-DIMENSIONAL SILHOUETTE SCORE ---
        try:
            sil_score = float(silhouette_score(X_embed, y_discrete_encoded))
            print(f"   | Silhouette Score{label_suffix}: {sil_score:.4f}")
        except Exception:
            sil_score = np.nan

        # --- METRIC 2: k-NN TOPOLOGICAL PROBE PURITY ---
        try:
            # 3-NN evaluated via 5-Fold Stratified Cross-Validation
            knn = KNeighborsClassifier(n_neighbors=3, metric="euclidean", n_jobs=-1)
            cv_strategy = StratifiedKFold(
                n_splits=5, shuffle=True, random_state=RANDOM_STATE
            )
            knn_scores = cross_val_score(
                knn, X_embed, y_discrete_encoded, cv=cv_strategy, scoring="accuracy"
            )
            knn_purity = float(knn_scores.mean())
            print(f"   | k-NN Neighborhood Purity{label_suffix}: {knn_purity:.4f}")
        except Exception as e:
            print(f"   | k-NN Probe Failed: {e}")
            knn_purity = np.nan

        # --- VISUALIZATION LAYER: UMAP REDUCTION ---
        print("   | Constructing topological visualization map...")
        reducer = umap.UMAP(
            n_components=2,
            n_neighbors=15,
            min_dist=0.1,
        ...
        ax.set_title(f"UMAP Latent Geometry: {dataset_name}", weight="bold", pad=10)
        ...
        stat_box_text = (
            f"High-D Geometry Profile:\n"
            f"• Silhouette: {sil_str}\n"
            f"• k-NN Purity: {knn_str}"
        )
        ax.text(
            0.03,
            0.03,
        ...
        png_path = OUTPUT_DIR / f"{dataset_name}_geometry_profile.png"
        ...
        del X_embed, y, y_discrete, y_discrete_encoded, X_umap, current_record, archive
        gc.collect()

    print("\n" + "=" * 70)
    print(f"Execution Complete! All topological bounds mapped and strictly logged to:")
    print(f"{SUMMARY_CSV}")
    print("=" * 70)
