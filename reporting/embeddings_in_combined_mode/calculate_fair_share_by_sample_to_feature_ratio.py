"""
This script analyzes the relationship between the Sample-to-Feature Ratio (n/p)
and GBDT feature reliance (Fair Share Gap and Multiplier).
It generates scatter plots using a log-scaled X-axis and calculates the Spearman rank correlation.
"""
...
OUTPUT_GAP_SCATTER_RELATIVE = "outputs/sample_to_feature_ratio_gap_scatter.png"
OUTPUT_MULTIPLIER_SCATTER_RELATIVE = (
    "outputs/sample_to_feature_ratio_multiplier_scatter.png"
)
OUTPUT_CSV_RELATIVE = "outputs/sample_to_feature_ratio_correlation_summary.csv"
# ==============================================================================
...
def prepare_ratio_data(df):
    """Calculates Fair Share metrics and computes the Sample-to-Feature ratio."""
    ...
    # Calculate Sample-to-Feature Ratio (n/p)
    safe_features = combined_df["dataset_raw_features_count"].replace(0, np.nan)
    combined_df["Sample-to-Feature Ratio"] = (
        combined_df["dataset_samples_count"] / safe_features
    )
...
def generate_gap_scatter_plot(df, save_path):
    """Generates a scatter plot of Sample-to-Feature Ratio vs. Fair Share Gap."""
    ...
    plt.xlabel("Sample-to-Feature Ratio (n/p) (Log Scale)", fontsize=11)
    ...
    plt.close()
    print(f"Generated Ratio Gap Scatter plot: {save_path}")


def generate_multiplier_scatter_plot(df, save_path):
    """Generates a log-log scatter plot of Sample-to-Feature Ratio vs. Fair Share Multiplier."""
    plt.figure(figsize=(11, 7))
    ...
    plt.savefig(save_path, dpi=300)
    plt.close()
    print(f"Generated Ratio Multiplier Scatter plot: {save_path}")


def generate_correlation_csv(df, save_path):
    """Calculates Spearman correlation between n/p ratio and metrics."""
    summary_data = []

    # Global correlation
    overall_gap_corr, overall_gap_p = spearmanr(
        df["Sample-to-Feature Ratio"], df["Fair Share Gap"]
    )
    overall_mult_corr, overall_mult_p = spearmanr(
        df["Sample-to-Feature Ratio"], df["Fair Share Multiplier"]
    )
    ...
    for algo in ALGO_MAP.values():
        ...
        gap_corr, gap_p = spearmanr(
            algo_df["Sample-to-Feature Ratio"], algo_df["Fair Share Gap"]
        )
        mult_corr, mult_p = spearmanr(
            algo_df["Sample-to-Feature Ratio"], algo_df["Fair Share Multiplier"]
        )
...
def main():
    ...
    processed_df = prepare_ratio_data(df)
