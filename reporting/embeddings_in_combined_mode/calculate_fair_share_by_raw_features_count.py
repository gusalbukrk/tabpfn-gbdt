"""
This script analyzes the relationship between the original raw feature count
(inherent dataset dimensionality) and GBDT feature reliance (Fair Share Gap and Multiplier).
...
OUTPUT_GAP_SCATTER_RELATIVE = "outputs/raw_features_gap_scatter.png"
OUTPUT_MULTIPLIER_SCATTER_RELATIVE = "outputs/raw_features_multiplier_scatter.png"
OUTPUT_CSV_RELATIVE = "outputs/raw_features_correlation_summary.csv"
# ==============================================================================
...
def prepare_dimensionality_data(df):
    """Calculates Fair Share metrics using original raw feature counts."""
...
def generate_gap_scatter_plot(df, save_path):
    """Generates a scatter plot of Original Raw Feature Count vs. Fair Share Gap."""
    ...
    # Set X-axis to logarithmic scale for feature counts
    ax.set_xscale("log")

    plt.title("")
    plt.ylabel("Fair Share Gap (% Points)\n[> 0 indicates Over-weighting]", fontsize=11)
    plt.xlabel("Original Raw Feature Count (Log Scale)", fontsize=11)
    ...
    plt.close()
    print(f"Generated Dimensionality Gap Scatter plot: {save_path}")


def generate_multiplier_scatter_plot(df, save_path):
    """Generates a log-log scatter plot of Original Raw Feature Count vs. Fair Share Multiplier."""
    plt.figure(figsize=(11, 7))
    ...
    plt.savefig(save_path, dpi=300)
    plt.close()
    print(f"Generated Dimensionality Multiplier Scatter plot: {save_path}")


def generate_correlation_csv(df, save_path):
    """Calculates Spearman correlation between raw feature count and metrics."""
    summary_data = []

    # Global correlation
    overall_gap_corr, overall_gap_p = spearmanr(
        df["dataset_raw_features_count"], df["Fair Share Gap"]
    )
    overall_mult_corr, overall_mult_p = spearmanr(
        df["dataset_raw_features_count"], df["Fair Share Multiplier"]
    )
    ...
    for algo in ALGO_MAP.values():
        ...
        gap_corr, gap_p = spearmanr(
            algo_df["dataset_raw_features_count"], algo_df["Fair Share Gap"]
        )
        mult_corr, mult_p = spearmanr(
            algo_df["dataset_raw_features_count"], algo_df["Fair Share Multiplier"]
        )
...
def main():
    ...
    processed_df = prepare_dimensionality_data(df)
