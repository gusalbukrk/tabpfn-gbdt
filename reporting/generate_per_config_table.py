...
SUMMARY_CSV = SCRIPT_DIR / "leaderboards" / "matrix.csv"
...
\\toprule
Variant & Overall & Classification $\\scriptstyle ({cls_count}/{total})$ & Regression $\\scriptstyle ({reg_count}/{total})$ & Binary $\\scriptstyle ({bin_count}/{total})$ & Multiclass $\\scriptstyle ({mc_count}/{total})$ & Small $\\scriptstyle ({small_count}/{total})$ & Medium $\\scriptstyle ({medium_count}/{total})$ \\\\
\\midrule
