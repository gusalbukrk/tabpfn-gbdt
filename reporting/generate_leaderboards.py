# =====================================================================
# METRIC DESCRIPTIONS
# 1. Average Rank: The mean finishing position (1st, 2nd, 3rd) across all datasets. It neutralizes the scale differences between metrics like RMSE and Log Loss.
# 2. Improvability %: The percentage gap between a model's error and the absolute best error achieved on that dataset. (0% = you are the best; higher = you are further from the leader).
# 3. Normalized Score: A relative score scaled from 0.0 to 1.0, where 0.0 represents the median performance on that dataset, and 1.0 represents the absolute best performance.

# every cross-dataset aggregation happens only on quantities that were first normalized within each dataset
# A 10% improvability gap in log loss and a 10% improvability gap in RMSE don't necessarily reflect equivalent real-world difficulty-to-close
# this isn't something the code gets wrong — it's an inherent limit of percentage-based normalization across non-commensurable metrics.
#  build separate classification/regression leaderboards is a good move precisely because it sidesteps that nuance entirely
# =====================================================================
...
OUTPUT_DIR = SCRIPT_DIR / "leaderboards"
...
# =====================================================================
print("Cleaning up old leaderboard artifacts...")
artifacts = (
...
SLIM_CSV_PATH = OUTPUT_DIR / "matrix.csv"
...
expected_rows = len(all_datasets) * len(all_algos)
...
if len(df_slim) != expected_rows:
    ...
    # sys.exit(1) # Commented out temporarily for incomplete matrix processing
    print(
        "\n[WARNING] Proceeding with an incomplete matrix for preliminary generation..."
    )

print(
    f"Matrix validation passed: {len(all_datasets)} datasets x {len(all_algos)} algorithms perfectly aligned.\n"
)
...
def generate_leaderboard(
    ...
    # --- PER-DATASET METRIC TRANSFORMATION ---
    ...
    # Re-initialize rng for each metric so CI bounds are evaluated on identical bootstrap realities
    ...
    # Return full stats dictionaries for the macro-summary matrices
    return {
        "rank": rank_stats,
...
def format_strategy_name(algo_string):
    """Maps internal strategy strings to polished HTML representations with icons."""
...
def get_gradient_color(val, min_val, max_val, higher_is_better):
    """Calculates the specific hex color for a cell based on its float value."""
    if pd.isna(val):
        return "#1a1b26"
...
def export_styled_html(df_text, df_means, out_filename, metric_key):
    """Wraps the matrix DataFrame into a polished, responsive HTML table."""
...
# =====================================================================
# 8. PER-DATASET LEADERBOARDS
# =====================================================================
DATASET_LEADERBOARDS_DIR.mkdir(parents=True, exist_ok=True)
for dataset_name in df_slim["dataset"].unique():
    df_dataset = df_slim[df_slim["dataset"] == dataset_name]
    _ = generate_leaderboard(
        df_dataset,
        f"DATASET LEADERBOARD: {dataset_name.upper()}",
        DATASET_LEADERBOARDS_DIR / f"leaderboard_{dataset_name}.csv",
        include_ci=False,
    )


# =====================================================================
# 9. GENERATE HEAD-TO-HEAD WIN RATE MATRIX
# =====================================================================
print("\n" + "=" * 125)
print(" ⚔️ GENERATING HEAD-TO-HEAD WIN RATE MATRIX ⚔️")
print("=" * 125)
...
html_content = [
    ...
    "</style></head><body>",
    "<h2>Head-to-Head Win Rate Matrix</h2>",
...
win_html_path = OUTPUT_DIR / "matrix_win_rate.html"
with open(win_html_path, "w", encoding="utf-8") as f:
    f.write("\n".join(html_content))

win_csv_path = OUTPUT_DIR / "matrix_win_rate.csv"
win_matrix.to_csv(win_csv_path)

print(f"Win Rate Matrix exported to: {win_csv_path} and {win_html_path}")
